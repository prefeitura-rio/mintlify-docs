...
| Modelo | Quando usar |
|--------|-------------|
| `anthropic/claude-sonnet-4-6` | Padrão — desenvolvimento, revisão de código, refatorações complexas |
| `anthropic/claude-haiku-4-5` | Tarefas simples: explicar funções, formatar código, perguntas rápidas |
...
<Tip>
Para tarefas do dia a dia que não exigem raciocínio complexo — explicar um trecho de código, gerar uma mensagem de commit, formatar JSON — use `claude-haiku-4-5` ou `gemini-3.5-flash`. Reserve o Sonnet para desenvolvimento de features e revisões profundas.
</Tip>
